...
export function AdminFuelMemberDialog({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        toast({
          title: "Could not load Fuel config",
  ...
  const save = async (clear = false) => {
    ...
    try {
      ...
      toast({ title: clear ? "Fuel config cleared" : "Fuel config saved" });
      onOpenChange(false);
    } catch (err) {
      toast({
        title: "Save failed",
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <p className="text-xs text-muted-foreground">
              On-track band is [target − deficit, target]. Effective floor below {FUEL_SAFETY_FLOOR_CAL}{" "}
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => setPlanJson(JSON.stringify(DEFAULT_FUEL_MEAL_PLAN, null, 2))}
                >
                  Reset default
                </Button>
              </div>
