...
### 5.8 No default spinner at checkout; prefetch on Book intent

| | |
|---|---|
| **Feature** | Flexi options loading experience at checkout |
| **Type** | Enhancement |
| **Decided by** | You (13 Jul 2026) |
| **Factors** | Spinner spun on every checkout even when Flexi was never opened; wasted the gap between Book click and Flexi click |
| **Dependencies** | `client/src/lib/flexi-options.ts`, `client/src/lib/member-reserve-navigation.ts`, React Query cache |
| **Decision** | (1) No spinner by default. (2) Spinner shows only after the user clicks "Customise with Flexi" and options are not yet ready. (3) Both booking options are always open; the Flexi button is greyed only when there are genuinely no swaps (resolved, not loading). (4) Prefetch starts on Book intent (click/hover/focus) from Home, Dashboard, and Calendar, keyed by class id, so checkout paints cached results instantly. (5) When we already know there are no swaps, show the greyed button and sorry message with no spinner. |
| **classTypeId path** | Book-on-class-type has no concrete session id at click time, so it falls back to reserve's on-mount fetch by design. |
| **Staleness** | Cached options paint instantly; reserve revalidates capacity in the background (`refetchOnMount: "always"`, surfaced as `isFetching`, not `isLoading`). Query `staleTime` is 45s. |
| **Rationale** | Faster perceived checkout; no dead spinner; single query key means prefetch and checkout dedupe in cache |
| **Shapes product** | One shared flexi query + navigation primitive; every current and future discovery surface inherits prefetch |

### 5.9 Groundwork for Flexi-aware search (future scope)

| | |
|---|---|
| **Feature** | Search / filter / sort by "has Flexi swaps" and "most flexible" |
| **Type** | New (groundwork now, UI later) |
| **Decided by** | You (13 Jul 2026) |
| **Factors** | Search bar is on the roadmap; availability-based filtering would cause an N+1 storm if bolted on naively |
| **Dependencies** | `shared/flexi-discovery.ts`, `POST /api/flexi/eligibility/batch`, `storage.getFlexiEligibilitySummaries` |
| **Decision** | Lock the contract now: (1) Eligibility badge is client-side and free (`flexiEnabled` + `recurrenceKind` + `seriesId`). (2) Actual swaps/availability come from a batch endpoint returning lightweight summaries only (no full option payload), capped at 50 ids. (3) Prefetch fires on row intent (hover/focus/click), never on list render. (4) Any search/catalog payload must preserve the flexi eligibility fields. (5) "Most flexible" sort uses `flexibilityScore` via `compareFlexiFlexibility`. |
| **Rationale** | Search becomes a zero-rework consumer of existing primitives; avoids N+1 and retrofits |
| **Shapes product** | Discovery-led Flexi adoption; see `docs/FLEXI-DISCOVERY-ARCHITECTURE.md` |
